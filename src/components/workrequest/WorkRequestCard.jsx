/**
 * [V] VIEW COMPONENT: WorkRequestCard
 * Displays a single work request ticket in the list.
 */
import React from 'react';
import { Box, Typography } from '@mui/material';
import { Wrench, Warning, ArrowRight } from '@phosphor-icons/react';
import { WORK_REQUEST_STATUS } from '../../models/WorkRequestModel';

export function WorkRequestCard({ data }) {
  // Determine status color
  let statusColor = '#ffffff'; 
  let statusBg = '#94a3b8'; 

  switch (data.status) {
    case WORK_REQUEST_STATUS.WAITING_APPROVAL:
    case WORK_REQUEST_STATUS.ON_REVIEW:
      statusBg = '#f97316'; // Orange
      break;
    case WORK_REQUEST_STATUS.SURVEY_SCHEDULED:
    case WORK_REQUEST_STATUS.WORK_SCHEDULED:
      statusBg = '#a855f7'; // Purple
      break;
    case WORK_REQUEST_STATUS.ON_SURVEY:
    case WORK_REQUEST_STATUS.ON_WORK:
      statusBg = '#3b82f6'; // Blue
      break;
    case WORK_REQUEST_STATUS.COMPLETE:
      ...
      >
        ...
        <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start', mb: 1.5 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.2 }}>
            ...
            <Typography variant="body2" sx={{ color: '#94a3b8', fontSize: '0.85rem', display: '-webkit-box', WebkitLineClamp: 1, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
              {data.description}
            </Typography>
          </Box>
        </Box>
