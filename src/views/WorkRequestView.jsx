/**
 * [V] VIEW: WorkRequestView
 * View showing the list of Work Requests.
 */
...
export function WorkRequestView({ controller }) {
  const handleBack = () => {
    controller.setActiveTab('home');
  };

  const actionItems = workRequestMockData.filter(p => p.needAction);
  const regularItems = workRequestMockData.filter(p => !p.needAction);
      ...
      <Box sx={{ flexGrow: 1, overflowY: 'auto', p: 2, pb: 12 }}>
        ...
        {actionItems.length > 0 && (
          <Box sx={{ mb: 4 }}>
            ...
            <Box>
              {actionItems.map((wr) => (
                <WorkRequestCard key={wr.id} data={wr} />
              ))}
            </Box>
          </Box>
        )}
