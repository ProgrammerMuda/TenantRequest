/**
 * [V] VIEW: FitOutPermitView
 * View showing the list of fit out permits matching the requested design.
 */
...
import { PermitCard } from '../components/fitout/PermitCard';

export function FitOutPermitView({ controller }) {
  const handleBack = () => {
    controller.setActiveTab('home');
  };

  const actionItems = permitMockData.filter(p => p.needAction);
  const regularItems = permitMockData.filter(p => !p.needAction);
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', flexGrow: 1, overflow: 'hidden', backgroundColor: '#f1f5f9' }}>
      ...
      <Box sx={{ px: 2, py: 2, backgroundColor: '#ffffff', display: 'flex', gap: 1.5 }}>
        ...
        >
          ...
          <InputBase
            placeholder="Find permit number"
            sx={{ 
              ml: 1, 
              flexGrow: 1, 
              fontSize: '1rem', 
      ...
      {/* List Content */}
      <Box sx={{ flexGrow: 1, overflowY: 'auto', p: 2, pb: 4 }}>
        ...
        {actionItems.length > 0 && (
          <Box sx={{ mb: 4 }}>
            ...
            <Box>
              {actionItems.map((permit) => (
                <PermitCard 
                  key={permit.id} 
                  data={permit} 
                  onClick={() => controller.openPermitDetail(permit)} 
                />
              ))}
            </Box>
          </Box>
